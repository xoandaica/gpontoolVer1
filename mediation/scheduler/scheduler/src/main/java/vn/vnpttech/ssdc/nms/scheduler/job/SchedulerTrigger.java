...
public class SchedulerTrigger {
    ...
    private static SchedulerTrigger instance = null;

    protected SchedulerTrigger() {
        // Exists only to defeat instantiation.
    }

    public static SchedulerTrigger getInstance() {
        if (instance == null) {
            instance = new SchedulerTrigger();
        }
        return instance;
    }
...
//           PropertyConfigurator.configure("etc/log4j.conf");
            ...
            JobDetail job = JobBuilder.newJob(PolicyJob.class)
                    .withIdentity(jobKey).build();
            int intervalInt = Integer.parseInt(interval);
            Trigger trigger = TriggerBuilder
                    .newTrigger()
                    .withIdentity("stbTriggerName", "stbGroup")
            ...
            scheduler.getListenerManager().addJobListener(
                    new PolicyJobListener(), KeyMatcher.keyEquals(jobKey)
            );
            scheduler.start();
